      ...
      <div className="w-full max-w-3xl shadow-md rounded-lg px-4 py-6">
        ...
        {isLoading ? (
          ...
          <div className="border-t border-gray-300 pt-4">
            <div className="flex justify-between font-semibold text-sm md:text-base text-gray-700 flex-wrap">
              <div className="w-1/2 sm:w-1/4">Order #</div>
            ...
            <div className="mt-4">
              {orders.map((order, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center py-2 border-b border-gray-200 flex-wrap"
                >
                  <div className="w-1/2 sm:w-1/4 text-xs md:text-base font-medium">
                    {order.id}
                  </div>
                  <div className="w-1/2 sm:w-1/4 text-xs md:text-base">
                    {order.item_ordered}
                  </div>
                  {showAddressColumn && (
                    <div className="w-1/2 sm:w-1/4 text-xs md:text-base">
                  ...
                  {isAdmin && order.transaction_status !== "Completed" && (
                    <button
                      className="bg-green-600 hover:bg-green-700 text-white font-semibold py-1 px-3 rounded mt-2 sm:mt-0"
